...
exatamente um extremo em~$S$. Um \emph{corte} em~$G$ é um 
conjunto~$C$ de arestas tal que~${C =~\delta(S)}$ para algum 
conjunto~$S$ 
...
e~$\bar{S}$ tal que~$||S|- |\bar{S}|| \in \{0,1\}$, ou seja,~$S$ 
e~$\bar{S}$ particionam o grafo basicamente ao meio. 
...
\label{alg:simpleApproxCutForest}
	...
	\For{$i=1 \to k$}{
		...
		\ElseIf{$|B|<m$}{
			...
			$\frac{m-|B|}{2} <|B'| \le~m-|B|$ (usando o Lema \ref{lema:simpleApproxCutTree})}
...
\textbf{Análise do Algoritmo}
	...
	que satisfaz o lema, e podemos fazer isso utilizando o Lema 
	\ref{lema:simpleApproxCutTree}.
	...
	Algoritmo 
	\ref{alg:simpleApproxCutTree}, então essa operação pode 
...
\label{alg:approxCutForest}
	...
	\eIf{$c\le~\frac{1}{2}$}{
		...
		$\frac{m}{2} <|B'| \le~m$ (usando o Lema \ref{lema:simpleApproxCutForest})}
	...
	}
	{
		...
		\While{$|B|<cm$}{
			...
			$\frac{m-|B|}{2} <|B'| \le~m-|B|$ (usando o Lema \ref{lema:simpleApproxCutForest})}
...
\section {Rotulação dos vértices da árvore}
	...
	seus rótulos. Portanto temos 
	que para um vértice~$v$, ~$map^+_m(v) =~v+~m$ 
	e~$map^-_m(v) =~v-~m$ como simplificação das funções
