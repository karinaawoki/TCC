...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	que~$0<~diam^*(G)\le~1$, pois haverá no mínimo um vértice de~$G$ no
	caminho máximo, e no máximo, todos os vértices de~$G$.
...
\section {Rotulação dos vértices da árvore}
	...
	que para um vértice~$v$, ~$map^+_m(v) =~(v+~m)\mod n$ 
	e~$map^-_m(v) =~(v-~m)\mod n$ como uma simplificação das funções
	...
	\subsection{Teorema da dobra do diâmetro}
		...
		\label{teo:dobraDiametro}
			...
			\begin{enumerate}
				...
				$e_T(B,W,S)\le \dfrac{2\cdot \Delta(T)}{diam^*(T)}$, 
...
\section {Algoritmo FST para bissecção}
	...
	\begin{teo}[{\cite[Theorem 6]{Schmidt15}}]
	...
	\begin{proof}
		...
		Primeiramente, temos que~$0<diam^*(T)\le1$, como o mostrado na 
		seção~\ref{subsec:diametro}. 
		Sendo assim, sabe-se que~$i\ge1$,
		logo~$i=1$ será a base utilizada na indução.
		
		Caso~$i=1$.
		Para todo~$n\le 2$, sabe-se que~$|E(T)|=\Delta(T)$, e nesse
		caso,~$e_T(B,W)\le |E(T)|=\Delta(T)\le 4\cdot 2^i\cdot \Delta(T)$.
		Para~$n>2$ temos que~$diam^*(T)>\dfrac{1}{2}$, logo, 
		podemos utilizar o Lema~\ref{lema:caminhoLongo} para obter 
		um corte~$(B,W)$ com~$e_T(B,W)\le 2 < 4\cdot 2^i\cdot \Delta(T)$.
		Portanto, esse teorema é válido para~$i=1$.


		Caso~$i\ge 2$. Provaremos agora que esse teorema é válido 
		para~$i$, assumindo
		que ele vale para~$i-1$.
		De modo semelhante ao caso anterior, 
		para~$n\le2$, temos 
		que~$e_T(B,W)\le |E(T)|=\Delta(T)\le 2\cdot 2^i\cdot \Delta(T)$.
		Já se~$n>2$, podemos utilizar o Teorema~\ref{teo:dobraDiametro},
		que nos devolverá cortes do tipo 1 ou 2.
		\begin{itemize}
			\item Se for do tipo 1, teremos~$|B|=m$ 
			e~$e_T(B,W)\le2\le 2\cdot 2^i\cdot \Delta(T)$..

			\item Se for do tipo 2, podemos obter um corte~$(B',W',S')$
			...
			sabemos 
			que~$diam^*(T[S'])\ge 2\cdot diam^*(T)>\dfrac{1}{2^{i-1}}$.
			Como~$0<m-|B'|\le|S'|$ e~$diam^*(T[S'])>\dfrac{1}{2^{i-1}}$,
			podemos usar o Teorema~\ref{teo:dobraDiametro} 
			em~$T[S]$ com~$m$ valendo~$m-|B'|$ e~$i$ valendo~$i-1$.
			
			Porém, existem casos onde o conjunto~$S'$ retornado pelo
			Teorema~\ref{teo:dobraDiametro} não é conexo, pois ele
			pode se dividir em duas componentes, uma no começo e outra
			no fim do caminho máximo. 
			Como o Teorema~\ref{teo:corteExato} se trata de árvores, 
			precisamos tornar~$S'$ conexo de forma a não alterar 
			o~$\Delta(T)$.
			Sabemos que se adicionármos uma aresta~$e$ em~$T$,
			que liga o primeiro ao último vértice do caminho máximo,
			então~$S'$ será conexo. 
			Diremos que~$T' = T\cup e$.
			De acordo com o Algoritmo~\ref{alg:dobraDiametro},
			sabemos que, caso~$S'$ seja desconexo, ele será como o 
			...
			\begin{center} \begin{tikzpicture}
				...
				\draw [draw=blue!65,fill=yellow!20] (-1,1.5) rectangle (7, 6.5);

				\draw [draw=none, fill=yellow!20] (11,4.7) rectangle (19, 6.5);
				\draw [draw=none, fill=yellow!20] (13.5,1.5) rectangle (19, 6.5);

				\node [text=blue!65,draw=none, fill=none] at (0,2.5) {\Large{$S$}};
				\node [text=blue!65,draw=none, fill=none] at (18,2.5) {\Large{$S$}};
				...
				\draw[draw=blue!65] (P0) -- (P1) -- (P2) -- (P3) -- (P4) -- (P5) -- cycle;


			\end{tikzpicture} \end{center}

			Portanto nota-se que em ambas
			as componentes de~$S'$, temos que o caminho máximo de cada uma
			é a interseção entre o caminho máximo de~$T$ e a componente,
			logo, se adicionando~$e$, teremos um caminho máximo em~$S'$ e preservaremos, em parte, o caminho máximo de~$T$.

			Somente os vértices dos extremos terão o seu grau alterado, e
			ainda sabemos que esses vértices são folhas em~$T$, logo, 
			possuem grau. 
			Quando acrescentarmos~$e$, o grau de ambos passará a ser 2,
			e como nesse caso~$n>2$, então~$\Delta(T)\ge 2$. 
			Portanto,~$\Delta(T')=\Delta(T)$.
			Agora podemos passar a árvore~$T'[S']$ para o 
			...
			para~$i-1$, então ele nos devolverá um corte~$(B'',W'')$
			tal que~$|B''|=m-|B'|$ 
			e~$e_{T[S]}(B'',W'')\le 4\cdot 2^{i-1}\cdot\Delta(T'[S'])
			\le 4\cdot 2^{i-1}\cdot\Delta(T')$.
			Dessa forma, podemos devolver o 
			corte~($B'\cup B'', V(T)\setminus (B'\cup B''))$ 
			como~$(B,W)$, tendo assim,~$|B|=|B'| + m-|B'| = m$ e
			\begin{align}
			...
			para~$i-1$. Logo, temos que o Teorema~\ref{teo:corteExato}
			é válido para todo~$i\in \mathbb{N^+_*}$.
	...
	\subsection{Algoritmo do corte exato}
		...
		\label{alg:corteExato}
			...
			\While{$|B|<m$}
			{
				$(B',W',S')\gets$ Algoritmo\ref{alg:dobraDiametro}$(T\setminus bça, m-|B| )$\;
				$B\gets B\cup B'$\;
			}
